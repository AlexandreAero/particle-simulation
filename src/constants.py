FPS            = 60
CELL_SIZE      = 5
SCREEN_SIZE    = (600, 600)
CLEAR_COLOR    = (0, 0, 0)
MATERIAL_FILE  = './materials.json'

BRUSH_SMALL = 'brush_small'
BRUSH_BIG   = 'brush_big'

MATERIAL_NAME_NONE     = 'none'
MATERIAL_NAME_SAND     = 'sand'
MATERIAL_NAME_WATER    = 'water'
MATERIAL_NAME_LAVA     = 'lava'
MATERIAL_NAME_ACID     = 'acid'
MATERIAL_NAME_TOXICGAS = 'toxicgas'

# Materials can have three different types as of now.
MATERIAL_TYPE_NONE   = 'none' # Used for empty cells/particles
MATERIAL_TYPE_SOLID  = 'solid'
MATERIAL_TYPE_LIQUID = 'liquid'
MATERIAL_TYPE_GAS    = 'gas'